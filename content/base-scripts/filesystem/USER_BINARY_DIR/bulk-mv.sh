...
original_names_file="$(mktemp -p "$TMPDIR_CURRENT" bulk_mv_original_names.XXXXXX)"
changed_names_file="$(mktemp -p "$TMPDIR_CURRENT" bulk_mv_changed_names.XXXXXX)"
commands_file="$(mktemp -p "$TMPDIR_CURRENT" bulk_mv_commands.XXXXXX)"
